...
MAINTAINER Renaud GAGNON <renaud.gagnon@open-groupe.com>
...
ENV INSTALL_PATH /xls
# The name of the application is xls and while there
...
# having to CD into the /xls folder every time would be
# lame, so instead we can set the WORKDIR to be /xls.
#
...
# xls/ path inside of the Docker image.
#
# It copies it to /xls because of the WORKDIR being set.
#
...
# /xls folder inside of the Docker image.
#
...
#RUN bundle exec rake RAILS_ENV=production DATABASE_URL=postgresql://user:pass@127.0.0.1/dbname ACTION_CABLE_ALLOWED_REQUEST_ORIGINS=foo,bar SECRET_TOKEN=dummytoken
